...
function getBestValueScore(product) {
    ...
    const savingsRatio = originalPrice > price && originalPrice > 0
        ? ((originalPrice - price) / originalPrice) * 100
        : 0;

    return (discountPercent * 2.5) + savingsRatio + (rating * 4) + (price > 0 ? 10000 / price : 0);
}
...
export default function Products() {
        ...
        const rows = [
            ...
            },
            {
                ...
                values: compareProducts.map((product) => {
                    const savings = Number(product.original_price || 0) - Number(product.price || 0);
                    return savings > 0
            ...
            {
                label: 'Category',
                values: compareProducts.map((product) => product.category_name || 'General'),
            },
            {
                label: 'Seller',
                values: compareProducts.map((product) => product.seller_username || 'Paras Traders'),
            },
        ];
    ...
    const bestValueProductId = useMemo(() => {
        ...
        return [...compareProducts]
            .sort((left, right) => getBestValueScore(right) - getBestValueScore(left))[0]
            ?.id || null;
    }, [compareProducts]);
    ...
    const handleToggleCompare = (product) => {
        setCompareProducts((prev) => {
            ...
            if (prev.length >= MAX_COMPARE_ITEMS) {
                toast.info(`You can compare up to ${MAX_COMPARE_ITEMS} products at a time`);
                return prev;
            }
            return [...prev, product];
        });
    ...
    const selectedCategory = categories.find((category) => String(category.id) === String(categoryFilter));
            ...
            {compareProducts.length > 0 && (
                <section className="compare-tray">
                    <div className="compare-tray-copy">
                        ...
                        <h2>Selected products ({compareProducts.length}/{MAX_COMPARE_ITEMS})</h2>
                        <p>
                    ...
                    <div className="compare-tray-actions">
                        <button type="button" className="compare-link-btn" onClick={clearCompare}>
                            Clear list
                        </button>
                        <button
                            type="button"
            ...
            {compareProducts.length >= 2 && (
                <section id="comparison-panel" className="comparison-panel">
                    <div className="comparison-panel-header">
                        <div>
                            <p className="compare-kicker">Smart Price Comparison + Transparency</p>
                            <h2>Side-by-side comparison</h2>
                            <p>
                                Highlighted rows show where products differ. The blue badge marks
                                the strongest value pick from your current shortlist.
                            </p>
                        </div>
                        <button type="button" className="compare-link-btn" onClick={clearCompare}>
                            Reset comparison
                        ...
                        <table className="comparison-table">
                            ...
                            <tbody>
                                ...
                                {comparisonRows.map((row) => (
                                    <tr
                                        key={row.label}
                                        className={row.isDifferent ? 'comparison-different-row' : 'comparison-same-row'}
                                    >
                                        <th>{row.label}</th>
                                        {row.values.map((value, index) => (
                                            <td key={`${row.label}-${compareProducts[index]?.id || index}`}>{value}</td>
                                        ))}
                                    </tr>
                                ))}
                            </tbody>
            ...
            <div className="products-layout">
                <aside className={`filters-sidebar ${showFilters ? 'open' : ''}`}>
                    <div className="filter-header">
                        <h3>Filters</h3>
                        <button onClick={() => setShowFilters(false)} className="close-filters"><FiX /></button>
                    </div>
                    ...
                    <div className="filter-group">
                        ...
                        {categories.map((category) => (
                            <label
                                key={category.id}
                                className={String(categoryFilter) === String(category.id) ? 'active' : ''}
                            >
                                <input
                                    type="radio"
                                    name="category"
                    ...
                    <div className="filter-group">
                        <h4>Sort By</h4>
                        <select value={sortBy} onChange={(e) => handleFilterChange('ordering', e.target.value)}>
                    ...
                    {(categoryFilter || searchQuery || minPrice || maxPrice || sortBy) && (
                ...
                <div className="products-main">
                    {loading ? (
                        ...
                        <div className="no-products">
                            ...
                            <button onClick={clearFilters} className="btn-primary-lg">Clear Filters</button>
                        </div>
                    ) : (
                    ...
                    {totalPages > 1 && (
                        <div className="pagination">
                            {page > 1 && (
                                <button onClick={() => handleFilterChange('page', page - 1)}>Previous</button>
                            )}
                            ...
                            {page < totalPages && (
                                <button onClick={() => handleFilterChange('page', page + 1)}>Next</button>
                            )}
                        </div>
                    )}
                </div>
